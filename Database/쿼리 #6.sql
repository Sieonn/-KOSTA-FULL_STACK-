-- sub query
-- SELECT colum1, cloum2,...
-- from table
-- where 조건연산자 (select column1, column2 ... from table where 조건);

-- =,<>(!=),<,<=, >, >= : 단일행 서브쿼리 연산자

-- emp테이블에서 clark보다 급여를 많이 받는 사원의 사번, 이름, 급여 조회
SELECT e.empno, e.ename, e.sal
FROM emp e
where sal >(SELECT sal FROM emp WHERE ename='CLARK')
ORDER BY 3;

-- emp 테이블에서 WORD보다 커미션이 작은 사원의 이름과 커미션 조회
SELECT ename, comm
FROM emp
WHERE ifnull(comm,0) >(SELECT comm FROM emp WHERE ename='ADMAS',0); 
...
-- emp 테이블에서AWRD보다ㅣ 커미션이 적은 사원의 이름과 커미션 조회
SELECT ename,comm
FROM emp
WHERE IFNULL(comn,0)<IFNULL((SELECT comn FROM emp WHERE ename='ADAMS'),0);
